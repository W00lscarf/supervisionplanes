...
APP_TITLE = "Plan Anual de Supervisión: Planificación y Reporte"
DB_PATH = "rrd_supervision.db"
UPLOAD_DIR = "uploads"
DIVISIONES_PATH = "divisiones_chile_utf8sig.csv"
...
# Placeholder: reemplaza por tu nómina oficial o cárgala desde CSV
MINISTERIOS = [
    "(Seleccionar)",
...
# --------------------------------------------------------------------------------------
# Instrumentos: ahora incluyen AMBITO + REQUIERE_ENTIDAD/TIPO_ENTIDAD
# ambito: Nacional | Regional | Provincial | Comunal | Sectorial
# --------------------------------------------------------------------------------------
INSTRUMENTOS = [
    # Direcciones Regionales (territoriales)
    ...
    ("SRRD-PLAN-SEC", "Plan", "Planes Sectoriales (Ministerios consignados en la PN-RRD)", "Sectorial", 1, "Ministerio", "Procedimiento PAS", "Subdirección de Reducción del Riesgo de Desastres"),
    ("SRRD-MAP-AMEN", "Mapa", "Mapas de Amenaza (Ministerios consignados en la PN-RRD)", "Sectorial", 1, "Ministerio", "Procedimiento PAS", "Subdirección de Reducción del Riesgo de Desastres"),

    # Territorial (RRD / Emergencia)
    ...
    ("SRRD-COGRID-NAC-MP", "Compromisos COGRID", "Compromisos COGRID Nacional (Mitigación/Preparación)", "Nacional", 0, None, "Procedimiento PAS", "Subdirección de Reducción del Riesgo de Desastres"),

    # Subdirección Gestión de Emergencias (nacional por defecto)
    ...
    ("SGE-COGRID-NAC-R", "Compromisos COGRID", "Compromisos COGRID Nacional (Respuesta)", "Nacional", 0, None, "Procedimiento PAS", "Subdirección de Gestión de Emergencias"),

    # Subdirección Desarrollo Estratégico (nacional por defecto)
...
def main():
    ...
    if not os.path.exists(DIVISIONES_PATH):
        st.error(f"No se encontró {DIVISIONES_PATH}. Pon el CSV en la misma carpeta del app.py o ajusta DIVISIONES_PATH.")
        st.stop()
    ...
    # ---------------- TAB 1 ----------------
    with tab1:
        st.subheader("Planificación (ex ante)")

        with st.form("form_planificacion", clear_on_submit=True):
            c1, c2, c3 = st.columns(3)

            with c1:
                dependencia = st.selectbox("Dependencia (Responsable de Supervisión)", DEPENDENCIAS)
            ...
            with c3:
                ...
                cargo = st.text_input("Cargo responsable (opcional)", value="")
                email = st.text_input("Email responsable (opcional)", value="")
    ...
    # ---------------- TAB 2 ----------------
    with tab2:
        st.subheader("Reporte / Confirmación (ex post)")
        ...
        if plan_df.empty:
            ...
            view["label"] = view.apply(
                lambda r: (
                    ...
                    f'{(r["entidad_objetivo"] or "-")} | {r["nombre_instrumento"]} | {r["anio"]} {r["periodo_planificado"]}'
                ),
                axis=1
            )
            ...
            with st.form("form_reporte", clear_on_submit=True):
                ...
                with r2:
                    ...
                    cargo_rep = st.text_input("Cargo responsable reporte (opcional)", value="")
                with r3:
                ...
                if submit_rep:
                    ...
                    evidencia_path = save_uploaded_file(evidencia_file) if evidencia_file else None
                    payload = {
    ...
    with tab3:
        ...
        if not dfp.empty:
            ...
            st.download_button("Descargar consolidado (CSV)", merged.to_csv(index=False).encode("utf-8"),
                               file_name="consolidado_planificacion_reporte.csv", mime="text/csv")

    st.caption("Instrumentos y responsables según Procedimiento PAS (matriz Dependencia → Instrumentos).")
